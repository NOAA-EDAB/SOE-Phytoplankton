; $ID:	SOE_PP_REQUIRED_STACKED.PRO,	2023-09-19-09,	USER-KJWH	$
  PRO SOE_PP_REQUIRED_STACKED, VERSION_STRUCT, DIR_DATA=DIR_DATA, PRODUCTS=PRODUCTS, SHAPEFILE=SHAPEFILE, SUBAREAS=SUBAREAS, SAVEFILES=SAVEFILES
...
;  IF ~KEYWORD_SET(VERSTR.INFO.STACKED) THEN MESSAGE, 'ERROR: Expecting version that uses "stacked" input files
  ...
  FOR SA=0, N_ELEMENTS(SHPFILES)-1 DO BEGIN ; LOOP THROUGH SUBAREA SHAPE FILES
    ...
    FOR PP=0, N_ELEMENTS(PPRODS)-1 DO BEGIN ; LOOP THROUGH PRODS
      ...
      DTSET = VERSTR.PROD_INFO.(OK).DATASET & TPSET = VERSTR.PROD_INFO.(OK).TEMP_DATASET
      DTPROD = VERSTR.PROD_INFO.(OK).PROD & TPROD = VERSTR.PROD_INFO.(OK).TEMP_PROD
      DVERSION = VERSTR.PROD_INFO.(OK).VERSION
      TPDTR = VERSTR.INFO.YEAR
      CASE VPROD OF
      ...
      FILES = GET_FILES(DTSET, PRODS=DTPROD, PERIODS='M', DATERANGE=YR, FILE_TYPE='STACKED_STATS', VERSION=DVERSION)
      
      ; Check the last file to see if it is complete, if not use the temp dataset to fill out the final year
      LFILE = GET_FILES(DTSET, PRODS=DTPROD, PERIODS='M', DATERANGE=TPDTR, FILE_TYPE='STACKED_STATS', VERSION=DVERSION)
      IF LFILE NE [] THEN BEGIN
        LD = STACKED_READ(LFILE)
        OK = WHERE(LD.DB.STATFILE EQ '',COUNT)
        IF COUNT GT 0 THEN FILES = [FILES,GET_FILES(TPSET, PRODS=TPROD, PERIODS='M', DATERANGE=TPDTR, FILE_TYPE='STACKED_STATS')]
      ENDIF ELSE FILES = [FILES,GET_FILES(TPSET, PRODS=TPROD, PERIODS='M', DATERANGE=TPDTR, FILE_TYPE='STACKED_STATS')]
