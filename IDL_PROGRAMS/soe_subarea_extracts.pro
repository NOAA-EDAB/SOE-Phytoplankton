...
;   Dec 23, 2020 - KJWH: Added IF NONE(VERSION)  THEN MESSAGE, 'ERROR: Must provide the SOE VERSION'
;   Dec 06, 2022 - KJWH: Changed OUTSTATS input to EXTRACT_STAT (outstats wasn't used) in order to set the data stat (e.g. MEAN) to be extracted
;
;-
; ****************************************************************************************************
  ...
  FOR V=0, N_ELEMENTS(VERSION)-1 DO BEGIN
    ...
    IF ~N_ELEMENTS(EXTRACT_STAT)   THEN ESTAT   = VERSTR.INFO.EXTRACT_STAT    ELSE ESTAT   = EXTRACT_STAT
    ...
    FOR A=0, N_ELEMENTS(PRODS)-1 DO BEGIN
      ...
      FOR D=0, N_ELEMENTS(DPRODS)-1 DO BEGIN
        ...
        DPRD = PSTR.PROD
        DSET = PSTR.DATASET
        DVER = PSTR.VERSION & IF DVER NE '' AND ~HAS(DVER,'V') THEN DIRVER='V'+DVER ELSE DIRVER=''
        DIR_EXTRACT = DIR_OUT + DSET + SL & DIR_TEST, DIR_EXTRACT
        DTR = GET_DATERANGE(DTR)
        FOR F=0, N_ELEMENTS(FILETYPES)-1 DO BEGIN
          ATYPE = FILETYPES[F]
          IF KEYWORD_SET(VERSTR.INFO.STACKED) THEN ATYPE = 'STACKED_' + ATYPE
      ...
      IF TPROD NE [] THEN BEGIN
        ...
        FOR F=0, N_ELEMENTS(FILETYPES)-1 DO BEGIN
          ATYPE = FILETYPES[F]
          IF KEYWORD_SET(VERSTR.INFO.STACKED) THEN ATYPE = 'STACKED_' + ATYPE
          FILES = GET_FILES(TSET, PRODS=TPRD, PERIODS=TPERS, FILE_TYPE=ATYPE, MAPS=TMAP, VERSION=DVER, COUNT=COUNT)
          FILES = DATE_SELECT(FILES,DATE_2YEAR(TDTR))
          ...
          SAV = REPLACE(SAV,['--','-.'],['-',''])
          SUBAREAS_EXTRACT, FILES, SHP_NAME=SHPFILE, SUBAREAS=NAMES, DATERANGE=TDTR, DIR_OUT=DIR_EXTRACT, STRUCT=STR, SAVEFILE=SAV, EXTRACT_STAT=ESTAT, /ADD_DIR
          EFILES = [EFILES,SAV]
        ENDFOR ; FILETYPE
        DATASETS = [DSET,TSET]
