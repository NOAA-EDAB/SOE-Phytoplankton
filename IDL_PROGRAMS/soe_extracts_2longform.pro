...
; ****************************************************************************************************
  ...
  FOR F=0, N_ELEMENTS(FILES)-1 DO BEGIN
    ...
    IF KEYWORD_SET(PPREQUIRED) THEN BEGIN
      DAT = DAT[WHERE(DAT.PROD EQ 'PPD')]
      SOEFILE = REPLACE(SOEFILE,'CHLOR_A-CCI-','')
      IF FILE_MAKE(FILES[F],SOEFILE,OVERWRITE=OVERWRITE) EQ 0 THEN CONTINUE                 ; Check if the file needs to be created
      DATTAGS = TAG_NAMES(DAT)
      STR = STRUCT_MERGE(STRUCT_COPY(DAT,TAGNAMES=['NAME','PERIOD','REGION','SUBAREA','TOTAL_PIXEL_AREA_KM2','N_MONTHS']),REPLICATE(STRUCT_2MISSINGS(CREATE_STRUCT('VARIABLE','','VALUE',0.0,'UNITS','','NOTES','')),N_ELEMENTS(DAT)))
      PRDS = ['PPD','CHLOR_A']
      ...
      FOR R=0, N_ELEMENTS(PRDS)-1 DO BEGIN
        ...
        FOR V=0, N_ELEMENTS(PVARS)-1 DO BEGIN
         ...
         ;   STRUCT_COPY(DAT, TAGNAMES=['PROD']),$
            ...
            STRUCT_COPY(DAT, TAGNAMES=['PROD_NAME','ALG_NAME','TIME_START','TIME_END','DURATION']),$
            STRUCT_COPY(DAT, TAGNAMES=['ALG_REFERENCE','SENSOR','SOURCE_DATA_VERSION','SOURCE_DATA_URL','SOURCE_DATA_DOI','CONTRIBUTOR_NAME']))
