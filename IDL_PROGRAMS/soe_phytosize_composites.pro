; $ID:	SOE_PHYTOSIZE_COMPOSITES.PRO,	2023-09-19-09,	USER-KJWH	$
...
; ****************************************************************************************************
  ...
  FOR V=0, N_ELEMENTS(VERSION)-1 DO BEGIN
    ...
    ALG = VALIDS('ALGS',VERSTR.PROD_INFO.PSC_MICRO.PROD)
    ...
    FOR C=0, N_ELEMENTS(COMP_PERIOD)-1 DO BEGIN
      ...
      FOR G=0, N_ELEMENTS(GROUP_PRODS)-1 DO BEGIN
        ...
        CASE GPRODS[0] OF
          'PSC_MICRO': BEGIN & CB_TITLE = UNITS('CHLOR_A') & CB_TICKS=[] & END
          'PSC_FMICRO': BEGIN & CB_TITLE = 'Fraction of Total Chlorophyll' & CB_TICKS=ROUNDS(FINDGEN(9)*0.1,1) & END
        ENDCASE
        ...
        FOR S=0, N_ELEMENTS(SETS)-1 DO BEGIN
          ...
          IF IS_STACKED(PSC[0]) THEN BEGIN
            STACKED_2PNGS, PSC[0], PRODS=PRODS, SPROD=SCL, DATERANGE=DR, MAP_OUT=MAP_OUT, OUTLINE=OUTLINE,OUT_COLOR=OCOLOR,/CURRENT,IMG_POS=[0,  85,256,341],/DEVICE,PAL=PAL, _EXTRA=_EXTRA
            STACKED_2PNGS, PSC[1], PRODS=PRODS, SPROD=SCL, DATERANGE=DR, MAP_OUT=MAP_OUT, OUTLINE=OUTLINE,OUT_COLOR=OCOLOR,/CURRENT,IMG_POS=[256,85,512,341],/DEVICE,PAL=PAL, _EXTRA=_EXTRA
            STACKED_2PNGS, PSC[2], PRODS=PRODS, SPROD=SCL, DATERANGE=DR, MAP_OUT=MAP_OUT, OUTLINE=OUTLINE,OUT_COLOR=OCOLOR,/CURRENT,IMG_POS=[512,85,768,341],/DEVICE,PAL=PAL, _EXTRA=_EXTRA
          ENDIF ELSE BEGIN
