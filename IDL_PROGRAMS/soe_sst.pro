...
; ****************************************************************************************************
  ...
  FOR V=0, N_ELEMENTS(VERSION)-1 DO BEGIN
    ...
    DATASETS = [VSTR.PROD_INFO.SST.DATASET,'AVHRR']
    ...
    FOR D=0, N_ELEMENTS(DATASETS)-1 DO BEGIN
      ...
      FOR R=0, N_ELEMENTS(PRODS)-1 DO BEGIN
        ...
        FOR T=0, N_ELEMENTS(TYPES)-1 DO BEGIN
          ...
          CASE TYPE OF 
            'STATS': BEGIN & PROD=VSTR.PROD_INFO.SST.PROD_SCALE & CB_TITLE=UNITS('TEMPERATURE') & SPAL=VSTR.PROD_INFO.SST.PAL & END
            'ANOMS': BEGIN & PROD=VSTR.PROD_INFO.SST.ANOM_SCALE & CB_TITLE=UNITS('TEMPERATURE') & SPAL=VSTR.PROD_INFO.SST.ANOM_PAL & END
          ENDCASE
          ...
          FOR M=0, N_ELEMENTS(VSTR.INFO.MAP_OUT)-1 DO BEGIN
            ...
            FOR F=0, N_ELEMENTS(FILES)-1 DO PRODS_2PNG,FILES[F],DIR_OUT=DIR_OUT,MAPP=OMAP,PROD=PROD,OUTLINE=EPU_OUTLINE,OUT_COLOR=0,OUT_THICK=4,CB_TITLE=CB_TITLE,ADD_TXT=TXT[F],TXT_POS=[0.98,0.02],TXT_ALIGN=1.0,TXT_SIZE=20,/ADD_CB,CB_RELATIVE=CB_RELATIVE,PAL=SPAL
