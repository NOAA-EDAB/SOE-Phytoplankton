; $ID:	SOE_WEEKLY_PLOTS.PRO,	2020-12-31-15,	USER-KJWH	$
  PRO SOE_WEEKLY_PLOTS, VERSTR, DATFILE=DATFILE, DIR_PLOTS=DIR_PLOTS, OVERWRITE=OVERWRITE, BUFFER=BUFFER
...
; ****************************************************************************************************
  ...
  IF NONE(VERSTR) THEN MESSAGE, 'ERROR: Must provide the SOE Version Structure'
  ...
  DIR_MOVIE = VERSTR.DIRS.DIR_PLOTS + SL + 'MOVIES' + SL & DIR_TEST, DIR_MOVIE
  NAMES = VERSTR.SHAPEFILES.(0).SUBAREA_NAMES
  TITLES = VERSTR.SHAPEFILES.(0).SUBAREA_TITLES
  MP = VERSTR.INFO.MAP_OUT
  DR = VERSTR.INFO.DATERANGE
  YEARS = YEAR_RANGE(DR[0],DR[1],/STRING)
  TEMP_DR = VERSTR.INFO.YEAR_DATERANGE
  ...
  FOR R=0, N_ELEMENTS(PLOT_PERIOD)-1 DO BEGIN
    ...
    ENDCASE
      ...
      FOR Y=0, N_ELEMENTS(YEARS)-1 DO BEGIN
        YR = YEARS[Y]
        PNGFILE = DIR_PLT + PER +'_'+YR + '-' + VERSTR.SHAPEFILES.(0).SHAPEFILE + '-' + STRJOIN(PRODS,'_') + '-TIMESERIES.png'
        PNGS = [PNGS,PNGFILE]
        ...
        FOR FTH=0L, N_ELEMENTS(NAMES)-1 DO BEGIN
          FOR PTH=0, N_ELEMENTS(PRODS)-1 DO BEGIN
            ...
            IF YR EQ TDP.YEAR THEN BEGIN ; Merge the temporary dataset with the primary dataset
              ...
              TSET = STRUCT[WHERE(STRUCT.SENSOR EQ PSTR.TEMP_DATASET AND STRUCT.MATH EQ 'STATS' AND STRUCT.SUBAREA EQ NAMES[FTH] AND STRUCT.PROD EQ VALIDS('PRODS',PSTR.TEMP_PROD) AND STRUCT.ALG EQ VALIDS('ALGS',PSTR.TEMP_PROD) AND STRUCT.PERIOD_CODE EQ PER,/NULL,TCOUNT)]
              IF TCOUNT EQ 0 THEN GOTO, NO_TEMPDATA
      ...
      FPS = 15
      MOVIE_FILE = DIR_MOVIE + 'SOE_'+VER + '-' + VERSTR.SHAPEFILES.(0).SHAPEFILE + '-' + STRJOIN(PRODS,'_') + '-' + CPER + '_TIMESERIES-FPS_'+ROUNDS(FPS)+'.webm'
