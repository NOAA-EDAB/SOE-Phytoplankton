  ...
  FOR V=0, N_ELEMENTS(VERSION)-1 DO BEGIN
    ...
    FOR SA=0, N_ELEMENTS(SHPFILES)-1 DO BEGIN ; LOOP THROUGH SUBAREA SHAPE FILES
      ...
      FOR PP=0, N_ELEMENTS(PPRODS)-1 DO BEGIN ; LOOP THROUGH PRODS
        ...
        IF KEYWORD_SET(VERSTR.INFO.STACKED) THEN BEGIN
          SOE_PP_REQUIRED_STACKED, VER, DIR_DATA=DIR_OUT, SHAPEFILE=SHPFILE, PRODUCTS=PPRODS[PP],SAVEFILES=SAVEFILES  ; Run the version that reads in the "stacked" files
          GOTO, MERGE_DATA
        ENDIF
